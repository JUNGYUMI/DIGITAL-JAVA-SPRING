	  ...
	  <table class="table table-bordered">
	    ...
	    <tbody>
	    	<c:forEach var="board" items="${list}">
	    		<tr>
		    		<td>${board.num}</td>
		    		<td><a href="<%=request.getContextPath()%>/board/detail?num=${board.num}&page=${pm.criteria.page}">${board.title}</a></td>
		    		<td>${board.writer}</td>
			...
			<form action="<%=request.getContextPath()%>/board/list">
				<div class="input-group mb-3">
					<select class="form-control"  id="sel1" name="type">
						<option value="0" <c:if test="${pm.criteria.type==0}"></c:if>>전체</option>
						<option value="1" <c:if test="${pm.criteria.type==1}"></c:if>>제목</option>
						<option value="2" <c:if test="${pm.criteria.type==2}"></c:if>>작성자</option>
						<option value="3" <c:if test="${pm.criteria.type==3}"></c:if>>내용</option>
					</select>
